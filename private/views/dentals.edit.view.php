...
<script>
    ...
    function filterTableByTooth(toothNumber) {
        tableRows.forEach(row => {
            const toothCell = row.cells[0]; // Assuming the tooth number is in the first cell
            if (toothCell.textContent.trim() === toothNumber) {
    ...
    teeth.forEach(tooth => {
        tooth.addEventListener('click', () => {
            const toothNumber = tooth.dataset.key;

            if (selectedTooth === tooth) {
                // Second click on the same tooth, reset
                selectedTooth.style.fill = '#FFFFFF';
                selectedTooth = null;
                teethNumberInput.value = '';
                resetTable();
            } else {
                // New tooth selected
                if (selectedTooth) {
                ...
                tooth.style.fill = '#7a5be9';
                selectedTooth = tooth;
                teethNumberInput.value = toothNumber;
                filterTableByTooth(toothNumber);
            }
        });
    });

    document.addEventListener('DOMContentLoaded', function() {
        // Check if there's a selected tooth number stored
        var selectedToothNumber = document.getElementById('tooth_number').value;

        if (selectedToothNumber) {
            // Find the tooth in the SVG that matches the stored number
            const tooth = document.querySelector(`.spots [data-key='${selectedToothNumber}']`);

            if (tooth) {
                // Apply the hover effect or any other indication
                tooth.style.fill = '#7a5be9'; // Example: change fill color
                selectedTooth = tooth; // Set the selected tooth globally if needed
                filterTableByTooth(selectedToothNumber); // Filter the table accordingly
            }
        }
    });
</script>
