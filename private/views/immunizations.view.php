...
<script>
    // Get references to the select element and the custom profession input field
    const professionSelectEdit = document.getElementById('vaccineSelectEdit');
    const customProfessionContainerEdit = document.getElementById('customVaccineContainerEdit');

    // Add an event listener to the select element
    professionSelectEdit.addEventListener('change', () => {
        // Check if the selected value is "Other"
        const selectedValue = professionSelectEdit.value;
        if (selectedValue === 'Other') {
            // If "Other" is selected, show the custom profession input field
            customProfessionContainerEdit.style.display = 'block';
        } else {
            // If any other option is selected, hide the custom profession input field
            customProfessionContainerEdit.style.display = 'none';
        }
    });
</script>
<script>
    // Get references to the select element and the custom profession input field
    const professionSelect = document.getElementById('vaccineSelect');
    const customProfessionContainer = document.getElementById('customVaccineContainer');
...
<script>
    ...
    const vaccineSelectEdit = document.getElementById('vaccineSelectEdit');
    const doseSelectEdit = document.getElementById('doseSelectEdit');

    // Define the available dose options for each vaccine
    const doseOptionsEdit = {
        'BCG': ['1'],
    ...
    function updateDoseOptions() {
        const selectedVaccine = vaccineSelectEdit.value;

        // Clear the current options in the dose select
        doseSelectEdit.innerHTML = '<option value="" disabled selected>Select</option>';

        // Add the appropriate dose options based on the selected vaccine
        if (doseOptionsEdit[selectedVaccine]) {
            doseOptionsEdit[selectedVaccine].forEach((dose) => {
                const option = document.createElement('option');
                option.value = dose;
                option.textContent = dose;
                doseSelectEdit.appendChild(option);
            });
        }

        // Enable or disable the dose select based on the selected vaccine
        doseSelectEdit.disabled = !selectedVaccine;
    }

    // Add an event listener to the vaccine select to update the dose options
    vaccineSelectEdit.addEventListener('change', updateDoseOptions);
